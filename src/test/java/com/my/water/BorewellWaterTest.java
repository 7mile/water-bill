...
public class BorewellWaterTest {
    private final BorewellWater objectUnderTest = BorewellWater.instance;

    @Test
    public void shouldReturnCorrectPrices() {
        List<Price> actualPrices = objectUnderTest.getPrices();

        assertThat(actualPrices).asList().containsExactly(new Price(0, Integer.MAX_VALUE, 1.5));
    }

    @Test
    public void shouldReturnCorrectCost() {
        assertThat(objectUnderTest.totalCost(1000.0)).isEqualTo(1500.0);
